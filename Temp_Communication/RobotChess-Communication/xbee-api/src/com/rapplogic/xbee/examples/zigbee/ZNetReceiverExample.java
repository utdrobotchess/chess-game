			...
			while (true) {
				...
				try {
					...
					if (response.getApiId() == ApiId.ZNET_RX_RESPONSE) {
						...
						int[] str = response.getProcessedPacketBytes();
						System.out.println("\n ");
						char[] output = new char[response.getProcessedPacketBytes().length];
						for (int index = 0; index < response.getProcessedPacketBytes().length; index++)
						{
							if(index > 13 && index < response.getRawPacketBytes().length-2)
								output[index]=(char)str[index];
						}
						System.out.println(response.getRawPacketBytes().length);
						System.out.println(new String(output).substring(13, response.getRawPacketBytes().length-2));
						System.out.println("\n");
						...
						if (atResponse.getApiId() == ApiId.AT_RESPONSE) {
							...
							//for (int index = 0; index < 6; index++)
								//System.out.println("\n"+((AtCommandResponse)atResponse).getValue()[0]);
						} else {
