...
public class ChessBoardTest
{
    ...
    {
        final int testSquares[] = {0 , 1 , 4 , 6 , 7 ,
                                   8 , 9 , 12, 14, 15,
                                   32, 33, 36, 38, 39,
                                   48, 49, 52, 54, 55,
                                   56, 57, 60, 62, 63};
        
        final int expectedNeighbors[][] = {{-1, -1,  1,  9,  8, -1, -1, -1}, // 0
                                           {-1, -1,  2, 10,  9,  8,  0, -1}, // 1
        ...
        for (int i = 0; i < testSquares.length; i++) {
            for (int j = 0; j < 8; j++) {
                Square actualNeighbor = squares[testSquares[i]].getNeighbor(j);
                int actualIntValue = -1;
                
                if (actualNeighbor != null)
                    actualIntValue = actualNeighbor.getIntLocation();
                
                assertEquals(expectedNeighbors[i][j], actualIntValue);
            }
        }
    }
}
